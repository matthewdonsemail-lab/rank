...
You can also use the `Authenticated`, `Unauthenticated`, `AuthLoading` and `AuthRefreshing` helper components which use the `useConvexAuth` hook under the hood. `AuthRefreshing` renders when queries and mutations are paused for token refresh (generally a rare case).
...
![Convex dashboard dev deployment settings](/screenshots/storybook/pages_project_deployment_settings_environment_variables_auth_0_light.webp)
...
To configure your client you can use environment variables as well. The exact name of the environment variables and the way to refer to them depends on each client platform (Vite vs Next.js etc.), refer to our corresponding [Quickstart](/quickstart/overview.md) or the relevant documentation for the platform you're using.
...
Use the `.env.local` or `.env` file to configure your client when running locally. The name of the environment variables file depends on each client platform (Vite vs Next.js etc.), refer to our corresponding [Quickstart](/quickstart/overview.md) or the relevant documentation for the platform you're using:
