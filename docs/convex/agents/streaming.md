...
* `sendSources` includes source parts emitted by the model. It defaults to `false`.
...
The example uses the [Convex AI Gateway](/ai-gateway/overview.md); see [Getting Started](/agents/getting-started.md) for installation and requirements.
...
async function stream(ctx: ActionCtx, threadId: string, order: number) {
  ...
  const response = streamText({
    ...
    model: convexGateway("openai/gpt-5-mini"),
