...
## Defining tools with approval[​](#defining-tools-with-approval "Direct link to Defining tools with approval")
...
## Server-side flow[​](#server-side-flow "Direct link to Server-side flow")
...
You can approve a tool call and continue generation in the same server function. For example, an action can call `approveToolCall` and then immediately call `streamText` to resume — no separate scheduling step needed.

## Handling multiple tool calls[​](#handling-multiple-tool-calls "Direct link to Handling multiple tool calls")
...
## Client-side flow[​](#client-side-flow "Direct link to Client-side flow")
...
## Example files[​](#example-files "Direct link to Example files")
