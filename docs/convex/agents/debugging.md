...
## Tracing LLM calls with OpenTelemetry[​](#tracing-llm-calls-with-opentelemetry "Direct link to Tracing LLM calls with OpenTelemetry")

When you pass [`experimental_telemetry`](https://ai-sdk.dev/docs/ai-sdk-core/telemetry) to a generate or stream call, the AI SDK emits OpenTelemetry spans with the model, token usage, prompt, and response.
...
Then enable telemetry on the call, and flush the spans before the handler returns so their exports are [awaited](https://docs.convex.dev/understanding/best-practices/#await-all-promises):
...
See the full example in [debugging/telemetry.ts](https://github.com/get-convex/agent/blob/main/example/convex/debugging/telemetry.ts).
