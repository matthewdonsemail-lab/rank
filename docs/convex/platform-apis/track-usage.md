...
Convex exposes per-deployment usage data through [Log Streams](/production/integrations/log-streams/.md). Log streams emit structured events for every function execution and periodic snapshots of storage usage, with the same resource fields that drive Convex billing.
...
## Setting up a log stream per deployment[​](#setting-up-a-log-stream-per-deployment "Direct link to Setting up a log stream per deployment")

When you create a new deployment on behalf of a user, configure a log stream for it as part of the same provisioning step. Use the [Create log stream](/deployment-api/create-log-stream.md) endpoint, authenticated with a deploy key for the deployment.
...
Store the returned log stream `id` alongside your record of the deployment so you can [update](/deployment-api/update-log-stream.md) or [delete](/deployment-api/delete-log-stream.md) it later (for example, when an end user deletes their app).

note

Identify which deployment a payload came from using the deployment name in the payload.

## Using log stream events for usage attribution[​](#using-log-stream-events-for-usage-attribution "Direct link to Using log stream events for usage attribution")

Two event types from the [log streams reference](/production/integrations/log-streams/.md) carry the bulk of the usage signal:
