...
Generate images and videos from an [action](/functions/actions.md) with `convexGateway`. For installation and authentication, see [Getting started](/ai-gateway/setup.md#vercel-ai-sdk).

The examples below run inside an action handler.

## Generate images[​](#generate-images "Direct link to Generate images")
...
## Generate videos[​](#generate-videos "Direct link to Generate videos")
...
The call waits for the video, then downloads it. The default timeout is ten minutes; the download limit is 64 MiB. Save `video.uint8Array` in [file storage](/file-storage/store-files.md).
...
`providerOptions.convexGateway` accepts `resolution` (such as `720p`), `generate_audio`, `frame_images`, and `input_references`. Standard SDK options take precedence. Supported values depend on the [OpenRouter model](https://openrouter.ai/docs/guides/overview/multimodal/video-generation).

## Async videos[​](#async-videos "Direct link to Async videos")
...
For local development, omit `webhookUrl` and use the [status and download methods](#retrieve-the-video). Callbacks require a cloud deployment's HTTPS `convex.site` URL; localhost URLs are not accepted.
...
### Receive a callback[​](#receive-a-callback "Direct link to Receive a callback")

In an [HTTP action](/functions/http-actions.md), load the job using `requestId` and verify the raw request body:
...
### Retrieve the video[​](#retrieve-the-video "Direct link to Retrieve the video")
