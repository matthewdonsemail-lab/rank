...
Convex currently has an [*authentication framework*](/auth/overview.md) which verifies user identities. In the future we plan to add an *authorization framework* which will allow developers to define what data a user can access.
...
For periodic OLAP analyses, [download a backup](/database/backup-restore.md) as a zip file. For advanced use cases, where you need to continuously stream data, Convex exposes a [Fivetran connector](/production/integrations/streaming-import-export.md) to export Convex data to external analytics systems.
