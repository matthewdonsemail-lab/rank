...
Vector searches are billed in [query-GBs](/production/state/limits.md#search-pricing): each search counts the full size of the vector index it runs against, regardless of how many results it returns and regardless of which `filterFields` it uses.
