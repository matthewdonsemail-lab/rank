...
Search queries are billed in [query-GBs](/production/state/limits.md#search-pricing): each query counts the full size of the search index it runs against, regardless of how many results it returns and regardless of which `filterFields` it uses.
